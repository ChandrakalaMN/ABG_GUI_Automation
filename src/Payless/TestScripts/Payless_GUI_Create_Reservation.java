...
public class Payless_GUI_Create_Reservation
{
	...
	
	{
		try {
			...
			for (int k = 1; k <= 60; k++)
			{
				Payless_GUI_Create_Reservation avis = new Payless_GUI_Create_Reservation();
				
				ReadWriteExcel rwe = new ReadWriteExcel("C:\\Avis_GUI_Automation\\Payless\\Payless_GUITestData_CreateReservation.xlsx");
				...
				if (file.isDirectory()) {
					...
					String codte = rwe.getCellData("Payless_CR", k, 12);
					String cotme = rwe.getCellData("Payless_CR", k, 13);
					String insta = rwe.getCellData("Payless_CR", k, 14);
					String cidte = rwe.getCellData("Payless_CR", k, 15);
					String citme = rwe.getCellData("Payless_CR", k, 16);
					String carGrp = rwe.getCellData("Payless_CR", k, 17);
					String awd = rwe.getCellData("Payless_CR", k, 18);
					String FTN = rwe.getCellData("Payless_CR", k, 19);
					String cardname = rwe.getCellData("Payless_CR", k, 20);
					String cardNo = rwe.getCellData("Payless_CR", k, 21);
					// System.out.print("excel card number in script :"
					// +cardNo);
					String expireMonth = rwe.getCellData("Payless_CR", k, 22);
					String expireYear = rwe.getCellData("Payless_CR", k, 23);
					String reason = rwe.getCellData("Payless_CR", k, 24);
				...
				if (Execute.equals("Y"))
				{
					...
					/* Enter FTN */
					if (rwe.getCellData("Payless_CR", k, 19).isEmpty()) {
					...
					for (int i = 0; i < radio.size(); i++) {
						if ((radio.get(i).isDisplayed()) && (radio.get(i).isEnabled())) {
							...
							if (radio.get(i).isSelected()) {
								...
								if (rwe.getCellData("Payless_CR", k, 25).isEmpty()) {
									System.out.print("No Insurance selected");
								} else {
									String[] insVal = rwe.getCellData("Payless_CR", k, 25).split("-");
									for (String e : insVal) {
								 ...
								 * Add CounterProducts
								 */
								if (rwe.getCellData("Payless_CR", k, 26).isEmpty()) {
									System.out.print("No CounterProduct selected");
								} else {
									String[] cpVal = rwe.getCellData("Payless_CR", k, 26).split("-");
									for (String e : cpVal) {
								...
								functions.ScreenCapturedate(ScreenshotPath,TCName);
								rwe.setCellData("Payless_CR", k, 40, Resmsg); // write
																			// respopup
								System.out.println(Resmsg);											// in
								Thread.sleep(8000);											// excel
								String resno = Resmsg.substring(54);
								System.out.println(resno);
								Thread.sleep(5000);
								//String resno = Resmsg.startsWith("Number",65);
								rwe.setCellData("Payless_CR", k, 27, resno);
								Thread.sleep(1000);
								...
								if (rwe.getCellData("Payless_CR", k, 27).isEmpty()) {
									rwe.setCellData("Payless_CR", k, 41, "FAIL");
									test.log(Status.FAIL, "Fail");
								} else {
									rwe.setCellData("Payless_CR", k, 41, "PASS");
								...
								// table
						{
							...
							rwe.setCellData("Payless_CR", k, 40, RSerrmsg);
							functions.ScreenCapturedate(ScreenshotPath,TCName);
